...
factor_labeling = read.table("GoSeq_List.Low_vs_Reg_minus_High_vs_Reg.txt", row.names=1)
...
background = read.table("GoSeq_List.DE_P0.001_C2.txt", header=T, row.names=1)
...
GO_info = read.table("kegg_annotations.txt", header=F, row.names=1,stringsAsFactors=F)
...
go_enrich_filename = paste("GoSeq_List.Low_vs_Reg_minus_High_vs_Reg.txt", '.GOseq.enriched', sep='')
    ...
    go_depleted_filename = paste("GoSeq_List.Low_vs_Reg_minus_High_vs_Reg.txt", '.GOseq.depleted', sep='')
