...
if (file.exists("GO-0009765_List.gene.counts.matrix.RData") && ! NO_REUSE) {
    print('RESTORING DATA FROM EARLIER ANALYSIS')
    load("GO-0009765_List.gene.counts.matrix.RData")
} else {
    print('Reading matrix file.')
    primary_data = read.table("GO-0009765_List.gene.counts.matrix", header=T, com='', row.names=1, check.names=F, sep='\t')
...
write.table(data, file="GO-0009765_List.gene.counts.matrix.log2.centered.dat", quote=F, sep='	');
if (nrow(data) < 2) { stop("
...
heatmap_data = data
pdf("GO-0009765_List.gene.counts.matrix.log2.centered.genes_vs_samples_heatmap.pdf")
...
", "GO-0009765_List.gene.counts.matrix.log2.centered" ) )
dev.off()
