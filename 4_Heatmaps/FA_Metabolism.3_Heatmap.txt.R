...
if (file.exists("FA_Metabolism.3_Heatmap.txt.RData") && ! NO_REUSE) {
    print('RESTORING DATA FROM EARLIER ANALYSIS')
    load("FA_Metabolism.3_Heatmap.txt.RData")
} else {
...
write.table(data, file="FA_Metabolism.3_Heatmap.txt.log2.centered.dat", quote=F, sep='	');
if (nrow(data) < 2) { stop("
...
heatmap_data = data
pdf("FA_Metabolism.3_Heatmap.txt.log2.centered.genes_vs_samples_heatmap.pdf")
...
", "FA_Metabolism.3_Heatmap.txt.log2.centered" ) )
dev.off()
