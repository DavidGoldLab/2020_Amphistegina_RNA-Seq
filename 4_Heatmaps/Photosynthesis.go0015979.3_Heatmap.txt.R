...
if (file.exists("Photosynthesis.go0015979.3_Heatmap.txt.RData") && ! NO_REUSE) {
    print('RESTORING DATA FROM EARLIER ANALYSIS')
    load("Photosynthesis.go0015979.3_Heatmap.txt.RData")
} else {
    print('Reading matrix file.')
    primary_data = read.table("Photosynthesis.go0015979.2_TMM.txt", header=T, com='', row.names=1, check.names=F, sep='\t')
...
write.table(data, file="Photosynthesis.go0015979.3_Heatmap.txt.log2.centered.dat", quote=F, sep='	');
if (nrow(data) < 2) { stop("
...
heatmap_data = data
pdf("Photosynthesis.go0015979.3_Heatmap.txt.log2.centered.genes_vs_samples_heatmap.pdf")
...
", "Photosynthesis.go0015979.3_Heatmap.txt.log2.centered" ) )
dev.off()
