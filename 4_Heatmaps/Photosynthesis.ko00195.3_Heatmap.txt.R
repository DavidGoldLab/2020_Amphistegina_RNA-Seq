...
if (file.exists("Photosynthesis.ko00195.3_Heatmap.txt.RData") && ! NO_REUSE) {
    print('RESTORING DATA FROM EARLIER ANALYSIS')
    load("Photosynthesis.ko00195.3_Heatmap.txt.RData")
} else {
    print('Reading matrix file.')
    primary_data = read.table("Photosynthesis.ko00195.2_TMM.txt", header=T, com='', row.names=1, check.names=F, sep='\t')
...
write.table(data, file="Photosynthesis.ko00195.3_Heatmap.txt.log2.centered.dat", quote=F, sep='	');
if (nrow(data) < 2) { stop("
...
heatmap_data = data
pdf("Photosynthesis.ko00195.3_Heatmap.txt.log2.centered.genes_vs_samples_heatmap.pdf")
...
", "Photosynthesis.ko00195.3_Heatmap.txt.log2.centered" ) )
dev.off()
