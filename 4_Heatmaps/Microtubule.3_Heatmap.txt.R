...
if (file.exists("Microtubule.3_Heatmap.txt.RData") && ! NO_REUSE) {
    print('RESTORING DATA FROM EARLIER ANALYSIS')
    load("Microtubule.3_Heatmap.txt.RData")
} else {
    print('Reading matrix file.')
    primary_data = read.table("Microtubule.2_TMM.txt", header=T, com='', row.names=1, check.names=F, sep='\t')
...
write.table(data, file="Microtubule.3_Heatmap.txt.log2.centered.dat", quote=F, sep='	');
if (nrow(data) < 2) { stop("
...
heatmap_data = data
pdf("Microtubule.3_Heatmap.txt.log2.centered.genes_vs_samples_heatmap.pdf")
...
", "Microtubule.3_Heatmap.txt.log2.centered" ) )
dev.off()
